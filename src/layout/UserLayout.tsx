...
const UserLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [userpr, setUserpr] = useState<any>(null);
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        setUserpr(res && res.data ? res.data : res);
      } catch {
        setUserpr(null);
      }
    };
    ...
    >
      {isAuthenticated ? (
        <>
          ...
          >
            <Avatar
              src={userpr?.avatarUrl || undefined}
              icon={!userpr?.avatarUrl && <UserOutlined />}
              style={{
                marginRight: 8,
                background: userpr?.avatarUrl ? undefined : COLOR_ACCENT,
                verticalAlign: "middle",
              }}
            >
              {!userpr?.avatarUrl && userpr?.username ? userpr.username.charAt(0).toUpperCase() : null}
            </Avatar>
            <span style={{ fontWeight: 500 }}>{userpr?.username || "Chưa có tên"}</span>
          </Menu.Item>
          <Menu.Divider />
          <Menu.Item key="profile-link" icon={<UserOutlined />} onClick={() => navigate("/profile")}>
            Thông tin cá nhân
          </Menu.Item>
